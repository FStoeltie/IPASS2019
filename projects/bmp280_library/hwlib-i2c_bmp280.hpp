...
public:
   ...
   void read( 
      ...
      uint8_t test;
      primitives.read( first_read, &test, n );			  
      first_read = false;
      hwlib::cout << "test: " << hwlib::hex << test << "\n" << hwlib::flush;
   }
...
/// Multiple read calls can be done on the object.
/// The destructor will close the transaction.
class i2c_read_transaction_bmp280 {
private:

   i2c_primitives & primitives;    
   bool first_read;
   uint_fast8_t i2c_addr, reg_addr; 
public:
   
   /// create (start) a read transaction
   i2c_read_transaction_bmp280( 
      i2c_primitives & primitives,
      uint_fast8_t i2c_addr,
      uint_fast8_t reg_addr
   ): 
       primitives( primitives ),
       first_read( true ),
       i2c_addr( i2c_addr ),
   ...
   uint_fast8_t read( 
      ...
      primitives.write_start();
      primitives.write( i2c_addr << 1 );  
      if(primitives.read_ack())  {
      } 
      primitives.write(0xD0);
      primitives.write_stop();
      if(primitives.read_ack())  {
      } 
      primitives.write_stop();
      //hwlib::wait_us(1000);
      primitives.write_start();
      primitives.write( ( 0x76 << 1 ) | 0x01 ); 
      if(primitives.read_ack())  {
      }
      data = primitives.read_byte();
      primitives.write_stop();         
      first_read = false;
      return data;
   }
   ...
   void read_from_register(  
      ...
      primitives.write( i2c_addr << 1 );  
      if(primitives.read_ack())  {
         //hwlib::cout << "got ack 1\n" << hwlib::flush;
      } 
      primitives.write(reg_addr);
      primitives.write_stop();
      if(primitives.read_ack())  {
         //hwlib::cout << "got ack 2\n" << hwlib::flush;
      } 
      primitives.write_stop();
      //hwlib::wait_us(1000);
      primitives.write_start();
      primitives.write( ( i2c_addr << 1 ) | 0x01 ); 
      if(primitives.read_ack())  {
         //hwlib::cout << "got ack 3\n" << hwlib::flush;
      }
      primitives.read( first_read, data, n );           
      first_read = false;
      primitives.write_stop(); 
      //hwlib::cout << "data received value: " << hwlib::hex << data[0] << "\n" << hwlib::flush;
   }
   void read( 
      uint8_t data[], 
      size_t n 
   ){  
      //hwlib::cout << "Reading multiple bytes\n" << hwlib::flush;
      //reg_addr = 0xD0;
      primitives.write_start();
      primitives.write( 0x76 << 1 );  
      if(primitives.read_ack())  {
         //hwlib::cout << "got ack 1\n" << hwlib::flush;
      } 
      primitives.write(0xD0);
...
public:
   ...
   i2c_read_transaction_bmp280 readbmp280( uint_fast8_t a, uint_fast8_t reg_addr)  {
      return i2c_read_transaction_bmp280( primitives, a, reg_addr);
   }     
