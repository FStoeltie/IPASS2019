...
public:
    ...
    void test03(bmp280& bmp01)  {
        ...
        bmp01.setStandbyTime(STANDBY_TIME::BME280_US_500);
        for(int i = 0; i < 10; i++)  {
            hwlib::cout << "Standby time 400us: " << hwlib::dec << i << ", " << bmp01.getTemperature() << "\n" << hwlib::flush;  
            hwlib::wait_ms(100);  
        }

        bmp01.setStandbyTime(STANDBY_TIME::BME280_S_1);
        for(int i = 0; i < 10; i++)  {
            hwlib::cout << "Standby time 1s: " << hwlib::dec << i << ", " << bmp01.getTemperature() << "\n" << hwlib::flush;  
            hwlib::wait_ms(200);  
        }
